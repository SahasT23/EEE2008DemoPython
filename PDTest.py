...
class MotorSimulation:
    ...
    def simulate_system(self, Kp, Kd, reference):
        """Simulate a simple DC motor with PD control"""
        # Time array
        ...
        for i in range(1, len(time)):
            ...
            # (Using velocity directly is often more stable than computing error derivative)
            deriv_error[i] = (error[i] - error[i-1]) / self.time_step
            
            # Calculate PD terms
            p_term[i] = Kp * error[i]
            d_term[i] = Kd * (-velocity[i-1])  # Derivative term (using velocity)
            
            # Calculate control signal (PD controller)
            control_signal[i] = p_term[i] + d_term[i]
        ...
        return time, position, velocity, control_signal, error, p_term, d_term
    
    def create_simulation(self):
        """Create interactive simulation plot"""
        # Initial simulation
        time, position, velocity, control_signal, error, p_term, d_term = self.simulate_system(
            self.Kp, self.Kd, self.reference)
        ...
        ax_Kd = plt.axes([0.25, 0.16, 0.65, 0.03])
        ax_ref = plt.axes([0.25, 0.12, 0.65, 0.03])
        ax_inertia = plt.axes([0.25, 0.08, 0.65, 0.03])
        ax_friction = plt.axes([0.25, 0.04, 0.65, 0.03])
        ...
        # Add title
        self.fig.suptitle('PD Controller Simulation', fontsize=16)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DC Motor PD Controller Simulation")
